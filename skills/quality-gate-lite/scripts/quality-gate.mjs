#!/usr/bin/env node
// ponytail: verbatim copy of ../../quality-gate/scripts/quality-gate.mjs so the
// lite skill installs standalone. The engine is identical — lite differs only in
// the shipped config (five fundamental metrics). Sync with `diff` if the full
// script changes; keep them byte-identical except this header.
//
