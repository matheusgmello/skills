...
description: While implementing code, pause and question the user about decisions that encode business rules or architecture — new features, conditionals/branches, classes, clauses, or similar structures. Use automatically during implementation whenever such a decision is about to be made, not just when explicitly invoked, so the user stays aware of and aligned with the code being written.
---
